...
$id = intval($_POST['headerId'] ?? 0);
if ($id === 0) {
  echo json_encode(["success" => false, "message" => "Invalid header."]);
  exit;
}

// --- Step 1: Get all hrefs related to header, items, and sublists ---
$hrefs = [];
$sql = "
  SELECT href 
  FROM sidebar 
  WHERE id = $id 
     OR parent_id = $id 
     OR parent_id IN (SELECT id FROM sidebar WHERE parent_id = $id)
";
...
if ($deleteResult) {
  echo json_encode([
    "success" => true,
    "message" => "✅ Header, related items, sublists, and their folders were deleted (include/ folder kept safe)."
  ]);
} else {
  echo json_encode([
    "success" => false,
